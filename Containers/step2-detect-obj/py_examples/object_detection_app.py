import os,time,cv2,argparse,multiprocessing,io,numpy
import numpy as np
...
def show_results(img, results, img_width, img_height):
    ...
    ret, jpeg = cv2.imencode('.png', img_cp)
    producer.produce('analyzed', jpeg.tobytes())
    producer.flush()
...
if __name__ == '__main__':
    ...
    # Source stream
    consumer = Consumer({'group.id': 'detect', 'default.topic.config': {'auto.offset.reset': 'earliest'}})
    consumer.subscribe(['/demos/drone/drone1:resized'])
	...
	if not msg.error():
		...
		src_pil_image = Image.open(io.BytesIO(src_image_data))
		frame = cv2.cvtColor(numpy.array(src_pil_image), cv2.COLOR_RGB2BGR)
